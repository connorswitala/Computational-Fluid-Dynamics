	...
	double rho, double e)
{
	...
	auto it = std::upper_bound(rho_vec.begin(), rho_vec.end(), rho);
	int i = max(0, static_cast<int>(std::distance(rho_vec.begin(), it)) - 1);
...
Solver::Solver(const int Nx, const int Ny, const inlet_conditions& INLET, Grid& grid, BoundaryConditions BoundaryType, double CFL, double Tw, int& progress_update, bool& chemical_eqbm_enabled) : Nx(Nx), Ny(Ny), rho_vec(rho_vec), chemical_eqbm_enabled(chemical_eqbm_enabled),
...
i_plus_inviscid_Jacobians(i_plus_inviscid_Jacobians), j_plus_inviscid_Jacobians(j_plus_inviscid_Jacobians), i_minus_inviscid_Jacobians(i_minus_inviscid_Jacobians), i_rho_A(i_rho_A), j_rho_A(j_rho_A), i_rho_fluxes(i_rho_fluxes), j_rho_fluxes(j_rho_fluxes),
...
CFL(CFL), inner_residual(inner_residual), outer_residual(outer_residual), cell_thermo(cell_thermo), chem_lookup_table(chem_lookup_table), progress_update(progress_update) {
	...
	if (chemical_eqbm_enabled) {
		string chemistry_table = "C:/Users/Connor/source/repos/Directed Study Spring 2025/build/Chemical Equilibrium/Release/Chemical_Equilibrium_Lookup_Table.csv";  // path to your CSV file
...
void Solver::compute_dt() {
	...
	for (int i = 0; i < Nx; ++i) {
		for (int j = 0; j < Ny; ++j) {
			...
			c = computeSoundSpeed(cell_thermo[i + 1][j + 1]);
...
void Solver::solve_inviscid() {
	...
	string filename = "Chemical Equilibrium Inviscid " + to_string(Nx) + "x" + to_string(Ny) + "_" + gridtype + "_Solution.csv";
...
void Solver::write_2d_csv(const string& filename) {
	...
	for (int i = 0; i < Nx; ++i) {
		for (int j = 0; j < Ny; ++j) {
			...
			density = cell_thermo[i + 1][j + 1].rho;
			pressure = cell_thermo[i + 1][j + 1].p;
			a = computeSoundSpeed(cell_thermo[i + 1][j + 1]);
			u_velocity = U[i + 1][j + 1][1] / U[i + 1][j + 1][0];
			v_velocity = U[i + 1][j + 1][2] / U[i + 1][j + 1][0];
			Mach = sqrt(u_velocity * u_velocity + v_velocity * v_velocity) / a;
			Temperature = cell_thermo[i + 1][j + 1].T;
